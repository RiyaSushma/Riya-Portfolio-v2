...
// Get your free access key at https://web3forms.com — paste it in .env.local as NEXT_PUBLIC_WEB3FORMS_KEY
...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      subject: "Portfolio Contact — " + (form.elements.namedItem("name") as HTMLInputElement).value,
      from_name: "Riya Portfolio",
      // Honeypot — bots fill this in; real users leave it blank
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <AnimateIn delay={0.15}>
          ...
          >
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2">
              {/* Left: info */}
              <div className="p-8 sm:p-10 lg:p-14 border-b lg:border-b-0 lg:border-r border-[var(--line)]">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-3 font-serif italic text-xl sm:text-2xl text-[var(--ink)] border-b border-[var(--line-strong)] pb-1 hover:text-[var(--pink-soft)] hover:border-[var(--pink-soft)] transition-all duration-200 break-all"
                >
              ...
              {/* Right: form */}
              <form
                className="p-8 sm:p-10 lg:p-14 flex flex-col gap-6"
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="flex flex-col gap-2">
                    <label
                      ...
                      className="font-mono text-[10px] tracking-widest uppercase text-[var(--muted)]"
                    >
                      Name
                    </label>
                    <input
                      ...
                      className="bg-transparent border-0 border-b border-[var(--line)] text-[var(--ink)] placeholder:text-[var(--muted-2)] py-2.5 text-sm outline-none focus:border-[var(--pink-soft)] transition-colors duration-200 disabled:opacity-50"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    rows={4}
                    disabled={status === "loading" || status === "success"}
                    className="bg-transparent border-0 border-b border-[var(--line)] text-[var(--ink)] placeholder:text-[var(--muted-2)] py-2.5 text-sm outline-none focus:border-[var(--pink-soft)] transition-colors duration-200 resize-none disabled:opacity-50"
                  />
                </div>
                ...
                {status === "error" && (
                  ...
                  >
                    ...
                    <a href={`mailto:${hero.email}`} className="underline hover:opacity-80">
                      {hero.email}
                    </a>
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={status === "loading" || status === "success"}
                  className="self-start inline-flex items-center gap-3 px-6 py-3.5 rounded-full font-semibold text-sm text-[#050508] overflow-hidden relative group disabled:opacity-60 disabled:cursor-not-allowed transition-opacity duration-200"
        ...
        <div className="mt-16 pt-8 border-t border-[var(--line)] flex flex-wrap items-center justify-between gap-4">
          <p className="font-mono text-xs text-[var(--muted)] tracking-wider">
            © 2026 · Riya · Designed &amp; built from scratch.
          </p>
